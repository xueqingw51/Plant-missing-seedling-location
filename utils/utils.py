...
def coco_only_cotton(path='../coco/labels/train2017/'):  # from utils.utils import *; coco_only_people()
...
def kmean_anchors(path='./data/cotton.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
...
def distancing(cotton_coords, img, dist_thres_lim=(200,250)):
    ...
    # x_combs = list(itertools.combinations(cotton_coords, 2))
    radius = 5
    thickness = 2
    for x in x_combs:
        xyxy1, xyxy2 = x[0], x[1]
        ...
        if dist < dist_thres_lim[0]:
            ...
            for xy in x:
                ...
                if already_red[cntr] == 0:
                    ...
                    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
                    cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
                    cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [255, 0, 0], thickness=tf,
                                lineType=cv2.LINE_AA)
            cntr3 = ((cntr1[0]+cntr2[0]) // 2, (cntr1[1]+cntr2[1]) // 2 )
            cv2.circle(img, cntr3, radius, color, -1)
            cv2.putText(img, 'point',cntr3, 0, tl / 3, [0,0,0] ,
